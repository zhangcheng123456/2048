...
Page({
    ...
    for (var j = 0; j < 4; j++) {
      for (var i = 0; i < 4; i++) {
        if (chessboardDatas[i][j] == 0 ||
          (i > 0 && chessboardDatas[i][j]==chessboardDatas[i - 1][j]) ||
          (i < 3 && chessboardDatas[i][j]==chessboardDatas[i + 1][j]) ||
          (j > 0 && chessboardDatas[i][j]==chessboardDatas[i][j - 1]) ||
          (j < 3 && chessboardDatas[i][j]==chessboardDatas[i][j + 1])) {
          check = false;
        }
      }
    }
  ...
  turnRight: function() {
    ...
    for (var j = chessboardDatas.length - 1; j >= 1; j--) {
      for (var i = chessboardDatas.length - 1; i >= 0; i--) {
        if (chessboardDatas[i][j] == chessboardDatas[i][j - 1]) {
          addScore += chessboardDatas[i][j];
          chessboardDatas[i][j] = chessboardDatas[i][j] + chessboardDatas[i][j - 1];
  ...
  reorder_right: function(chessboardDatas) {
    for (var j = chessboardDatas.length - 2; j >= 0; j--) {
      for (var i = chessboardDatas[j].length - 1; i >= 0; i--) {
        var rowIndex = j;
